...
class ManualControlNode():
    def __init__(self):
        ...
        self.rc_in_buffer_ = CustomQueue(maxsize=20)
        self.mocap_state_buffer_ = CustomQueue(maxsize=20)
        self.wrench_buffer_ = CustomQueue(maxsize=20)
        ...
        self.time_out_ = np.array([0.015,
                                   0.015,
                                   0.015])
        self.sync_tol_ = 0.010
        self.loopback_ = 0.050
    ...
    def _rcCallback(self, msg:RCIn):
        with self.cv_:
            ...
            self.time_latest_[0] = max(self.time_latest_[0] - self.latency_[0]
                                       , rc_temp[0])
    ...
    def _mocapCallback(self, msg:Odometry):
        with self.cv_:
            ...
            self.time_latest_[1] = max(self.time_latest_[1] - self.latency_[1]
                                       , mocap_temp[0])
    ...
    def _wrenchCallback(self, msg:WrenchStamped):
        with self.cv_:
            ...
            self.time_latest_[2] = max(self.time_latest_[2] - self.latency_[2],
                                       wrench_temp[0])
    ...
    def _process_func(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.is_first_run is True) and (not self.mocap_state_buffer_.empty()):
                self.t_curr_ = self.mocap_state_buffer_.back()[0]
            ...
            with self.cv_:
                ...
                while self.t_prev_ + self.period_ <= w and steps < MAX_CATCHUP_STEPS:
                    ...
                    rc_ok = self._prepareBufferNear(self.rc_in_buffer_,
                                                self.t_curr_, self.loopback_)
                    mocap_ok = self._prepareBufferNear(self.mocap_state_buffer_,
                                                self.t_curr_, self.loopback_)
                    wrench_ok = self._prepareBufferNear(self.wrench_buffer_,
                                                 self.t_curr_, self.loopback_)
                    ...
                    steps = steps + 1
                self.cv_.notify_all()
    def _rc_update(self, rc_ok:bool,
                         mocap_ok:bool,
                         wrench_ok:bool):
        ...
        if not rc_ok:
            ...
            t_rc, rc_ch = self.rc_in_buffer_.back()
            self.RcConverter_obj.set_rc(rc_ch)
        ...
        if mode == FlightMode.KILL:
            ...
            return
        if mode != FlightMode.MANUAL_STAB:
            return
        ...
        state_curr = mocap_item[1:]
        wrench_curr = wrench_item[4:]

        ref = np.concatenate([a_des, np.array([z_des, dpsi_dt_des])])
        self.RcController_obj.set_ref_state(ref, state_curr, wrench_curr)
    ...
    def _prepareBufferNear(self, buffer, t_ref, loopback) -> bool:
        if buffer.empty():
            return False
        cutoff = t_ref - loopback
...
if __name__ == '__main__':
    ...
    rospy.spin()
    node.process_thread.join()#! /usr/bin/env python3.8
import os
import sys
...
class ManualControlNode():
    ...
    def _rc_update(self, rc_ok:bool,
        ...
        mocap_item = self.mocap_state_buffer_.front()
        wrench_item = self.wrench_buffer_.front()
